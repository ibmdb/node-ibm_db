...
module.exports.createDbSync = function (connStr, dbName, options)
{
  var db2Instance = new Database(options);
  db2Instance.createDbSync(connStr, dbName);
  return db2Instance;
}; // ibmdb.createDbSync

module.exports.dropDbSync = function (connStr, dbName, options)
{
  var db2Instance = new Database(options);
  db2Instance.dropDbSync(connStr, dbName);
  return db2Instance;
}; // ibmdb.dropDbSync
 ...
 * 
 */
Database.prototype.createDbSync = function (connStr, db_Name) {
  var self = this, result, dbName, codeSet = null, mode = null;

  var connectionOpen = self.openSync(connStr);
  ...
  else 
  {
    ...
    result = self.conn.createDbSync(dbName, codeSet, mode);

    return result;
  }
} // Database.createDbSync
 ...
 * 
 */
Database.prototype.dropDbSync = function (connStr, db_Name) {
  ...
  else
  {
    ...
    result = self.conn.dropDbSync(dbName);
