...
{
    var err, prm, paramtype, ctype, sqltype, datatype, data; 
    for (var i = 0; i < params.length; i++)
    {
        if(Object.prototype.toString.call(params[i]) == "[object Object]")
        {
            ...
            datatype = undefined; data = undefined; 
            prm = params[i];
            if(prm.ParamType)
            {
            ...
            if(prm.Data)
            {
                params[i] = [paramtype, ctype, sqltype, prm.Data];
                //console.log("For "+(i+1) + "th param = " + 
                //        [paramtype, ctype, sqltype]);
            }
            else
            {
                err = "Data is missing from " + prm;
                break;
            }
        }
...
Pool.prototype.open = function (connStr, callback) 
{
  ...
  {
    db = new Database({ odbc : self.odbc, systemNaming: self.options.systemNaming });
