...
module.exports.createDatabaseSync = function (connStr, dbName, options)
{
  var db2Instance = new Database(options);
  db2Instance.createDatabaseSync(connStr, dbName);
  return db2Instance;
}; // ibmdb.createDatabaseSync

module.exports.dropDatabaseSync = function (connStr, dbName, options)
{
  var db2Instance = new Database(options);
  db2Instance.dropDatabaseSync(connStr, dbName);
  return db2Instance;
}; // ibmdb.dropDatabaseSync
 ...
 * Create Database.
 * (createDatabaseSync)
 * args: db_Name, code_Set, mode_
 * (code_Set, mode_ are optional and default values will be null)
 * 
 * return: TRUE on success or FALSE on failure.
 * 
 */
Database.prototype.createDatabaseSync = function (connStr, db_Name) {
  var self = this, result, dbName, codeSet = null, mode = null;
  ...
  if (typeof(self.codeSet) !== 'undefined'){
  ...
  var connectionOpen = self.conn.openSync(connStr);
  ...
  else 
  {
    ...
    result = self.conn.createDatabaseSync(dbName, codeSet, mode);
    
    return result;
  }
} // Database.createDatabaseSync
 ...
 * 
 */
Database.prototype.dropDatabaseSync = function (connStr, db_Name) {
  var self = this, result, dbName;
  ...
  else
  {
    ...
    result = self.conn.dropDatabaseSync(dbName);

    return result;
  }
} // Database.dropDatabaseSync
