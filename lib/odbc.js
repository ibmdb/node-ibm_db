  ...
  self.queue.push(function (next) {
    function cbQuery (initialErr, result, outparams)
    {
      ...
      if (result && typeof(result) === 'object') {
        ...
        deferred ? deferred.reject(initialErr, resultset, sqlca) : cb(initialErr, resultset, sqlca);
        return next();
      }
      ...
      function fetchMore()
      {
        ...
        result.fetchAll(function (err, data, colcount) {
          ...
          if( err || initialErr )
          {
            ...
            sqlca.state = initialErr.state || err.state;
            sqlca.sqlcode = initialErr.sqlcode || err.sqlcode;
            deferred ? deferred.reject(initialErr || err) : cb(initialErr || err, resultset, sqlca);
            result.closeSync();
            initialErr = null;
            err = null;
            return next();
          ...
          catch (e)
          {
            moreResultsError = e;
            moreResults = false;
            sqlca.state = moreResultsError.state;
            sqlca.sqlcode = moreResultsError.sqlcode;
          }
