...
if(os.platform() == 'win32') {
	process.env.PATH = process.env.PATH+';'+path.resolve(__dirname, '../installer/clidriver/bin');
}
...
module.exports.openSync = function (connectionString, options) {
...
function Database(options) {
  ...
  if(!options.odbc) {
      options.odbc = new odbc.ODBC();
  }
  self.odbc = options.odbc;
